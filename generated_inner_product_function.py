...
def generate_inner_product_function_detailed(csv_path):
    """
    Generate an inner product multiplication function with detailed analysis.
    """
    ...
    for i in range(num_basis):
        print(f"=" * 80)
        print(f"Row {i}: Inner product of basis element '{basis_names[i]}' from multivector a")
        print(f"=" * 80)
        ...
        print(f"\n2. CONVERTED FORM (after applying inner product Cayley table):")
        converted_terms = []
        conversion_details = []
    ...
    print("=" * 80)
    print("FINAL GENERATED INNER PRODUCT FUNCTION")
    print("=" * 80)

    function_code = """def inner_product(a, b):
    \"\"\"
...
if __name__ == "__main__":
    try:
        function_code, basis_symbols, result_terms = generate_inner_product_function_detailed(
            "PGA2D-Inner-Product-Caley.csv")

        print(function_code)

        # Save to file
        with open("generated_inner_product.py", "w") as f:
            f.write("# Auto-generated inner product function for PGA2D\n")
            f.write("# Generated with detailed analysis\n\n")
            f.write(function_code)

        print(f"\nFunction saved to 'generated_inner_product.py'")

    except FileNotFoundError:
        print("CSV file not found. Please make sure 'PGA2D-Inner-Product-Caley.csv' exists.")
